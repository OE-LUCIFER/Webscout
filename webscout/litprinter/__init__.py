...
class LitPrinter:
    ...
    def _format_dict(self, d: dict, indent_level: int = 0) -> str:
        ...
        for i, (key, value) in enumerate(d.items()):
                ...
                if any(
                    ...
                    if len(value) > 80:
                        wrapped = textwrap.fill(
                            value, width=80, subsequent_indent=next_indent + "    "
                        )
                        formatted_value = (
                            f"{self.theme['str']}'{wrapped}'{Colors.RESET}"
                        )
                    else:
            ...
            lines.append(
                f"{next_indent}{formatted_key}{self.theme['colon']}: {Colors.RESET}{formatted_value}{comma}"
            )
    ...
    def _highlight_code(self, code: str, language: str = "python") -> str:
        ...
        for line in code.split("\n"):
            if "#" in line:
                ...
                line = (
                    code_part
                    + f"{self.syntax_theme['COMMENT']}#{comment_part}{Colors.RESET}"
                )

            line = re.sub(
                r'(".*?"|\'.*?\')',
                f"{self.syntax_theme['STRING']}\\1{Colors.RESET}",
                line,
            )

            line = re.sub(
                r"\b(\d+)\b", f"{self.syntax_theme['NUMBER']}\\1{Colors.RESET}", line
            )
            ...
            for keyword in keywords:
                line = re.sub(
                    f"\\b{keyword}\\b",
                    f"{self.syntax_theme['KEYWORD']}{keyword}{Colors.RESET}",
                    line,
                )
    ...
    def _format_markdown_stream(self, text: str) -> str:
        ...
        text = re.sub(
            r"^# (.+)$",
            f"{self.markdown_theme['H1']}🔥 \\1{Colors.RESET}",
            text,
            flags=re.M,
        )
        text = re.sub(
            r"^## (.+)$",
            f"{self.markdown_theme['H2']}✨ \\1{Colors.RESET}",
            text,
            flags=re.M,
        )
        text = re.sub(
            r"^### (.+)$",
            f"{self.markdown_theme['H3']}💫 \\1{Colors.RESET}",
            text,
            flags=re.M,
        )
        ...
        text = re.sub(
            r"\*\*\*(.+?)\*\*\*", f"{Colors.BOLD}{Colors.ITALIC}\\1{Colors.RESET}", text
        )
        ...
        text = re.sub(
            r"```(\w+)?\n(.*?)\n```",
            lambda m: self._highlight_code(m.group(2), m.group(1) or "text"),
            text,
            flags=re.S,
        )
        ...
        text = re.sub(
            r"\[(.+?)\]\((.+?)\)",
            f"{Colors.BLUE}{Colors.UNDERLINE}\\1{Colors.RESET}",
            text,
        )
        ...
        text = re.sub(
            r"^> (.+)$",
            f"{self.markdown_theme['QUOTE']}│ \\1{Colors.RESET}",
            text,
            flags=re.M,
        )
        ...
        text = re.sub(
            r"- \[ \] (.+)$",
            f"{self.markdown_theme['TASK']}☐ \\1{Colors.RESET}",
            text,
            flags=re.M,
        )
        text = re.sub(
    ...
    def _format_table_markdown(self, match) -> str:
        ...
        result.append(
            "│ " + " │ ".join(f"{h:<{w}}" for h, w in zip(header, widths)) + " │"
        )
        result.append("├" + "┼".join("─" * (w + 2) for w in widths) + "┤")
        # Rows
        for row in rows:
            result.append(
                "│ " + " │ ".join(f"{str(c):<{w}}" for c, w in zip(row, widths)) + " │"
            )
        result.append("└" + "┴".join("─" * (w + 2) for w in widths) + "┘")
    ...
    def table(
        self, headers: List[str], rows: List[List[Any]], style: Optional[str] = None
    ):
    ...
    def tree(self, data: Union[Dict, List], indent: int = 0):
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                self.print(
                    "  " * indent + "├─ " + str(key) + ":", style=self.theme["key"]
                )
