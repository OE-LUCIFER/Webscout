...
class LogLevel(Enum):
    ...
    def style(self, style: str, color_scheme: Dict[str, Tuple[int, int, int]]) -> str:
        ...
        return (
            f"{style_code}\033[38;2;{color[0]};{color[1]};{color[2]}m{str(self)}\033[0m"
        )
...
class LitLogger:
    ...
    def _format_message(self, level: LogLevel, message: str, **kwargs) -> str:
        ...
        params = {
            "time": time_str,
            "level": level.style(
                self.level_styles.get(level._name, "NORMAL"), self.color_scheme
            ),
            "name": self.name,
            "message": message,
            "file": filename,
    ...
    def _detect_level(self, message: str, **kwargs) -> LogLevel:
        ...
        if kwargs:
            ...
            status = (
                kwargs.get("status") or kwargs.get("code") or kwargs.get("http_status")
            )
            if status:
                try:
                    status = int(status)
...
if __name__ == "__main__":
    ...
    for scheme_name, scheme, format in schemes:
        logger = LitLogger(
            name=f"{test_name}_{scheme_name}", format=format, color_scheme=scheme
        )
