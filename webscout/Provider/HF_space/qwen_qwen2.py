    ...
    def create_sync_generator(
        ...
        **kwargs: Any,
        ...
        user_messages: List[Message] = [
            message for message in messages if message["role"] != Role.SYSTEM.value
        ]
        ...
        with requests.Session() as session:
            ...
            response = session.post(
                self.config.api_endpoint, headers=self.headers.join, json=payload_join
            )
            ...
            with session.get(
                url_data, headers=self.headers.data, params=params_data, stream=True
            ) as response:
                response.raise_for_status()
                            ...
                            try:
                                ...
                                if json_data.get("msg") == "process_generating":
                                    if (
                                        ...
                                        output_data: List[Any] = json_data["output"][
                                            "data"
                                        ]
                                        if (
                                            len(output_data) > 1
                                            and len(output_data[1]) > 0
                                        ):
                                ...
                                if json_data.get("msg") == "process_completed":
                                    if (
                                        ...
                                        if (
                                            ...
                                            if final_full_response.startswith(
                                                full_response
                                            ):
                                                final_full_response = (
                                                    final_full_response[
