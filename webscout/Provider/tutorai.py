...
class TutorAI(Provider):
    ...
    def ask(
        ...
        if attachment_path:
            if not os.path.isfile(attachment_path):
                raise FileNotFoundError(
                    f"Error: The file '{attachment_path}' does not exist."
                )
            try:
                files["attachment0"] = (
            ...
            try:
                with requests.post(
                    ...
                    for chunk in response.iter_content(
                        chunk_size=self.stream_chunk_size, decode_unicode=True
                    ):
                        if chunk:
        ...
        def for_non_stream():
            ...
            if not homeworkify_html:
                return {
                    "text": "No content found in the response"
                }  # Default in case content not found
            clean_text = homeworkify_html  # Removed html_to_terminal call
