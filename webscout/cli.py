...
def _print_data(data):
    ...
    if data:
        for i, e in enumerate(data, start=1):
            ...
            for j, (k, v) in enumerate(e.items(), start=1):
                if v:
                    width = (
                        300
                        if k
                        in ("content", "href", "image", "source", "thumbnail", "url")
                        else 78
                    )
            ...
            console.print(
                Panel(table, title=f"Result {i}", expand=False, style="green on black")
            )
            console.print("\n")
...
def chat(proxy: str = None, model: str = "gpt-4o-mini"):
    ...
    console.print(
        "[cyan]Type your message and press Enter. Press Ctrl+C or type 'exit' to quit.[/]\n"
    )
...
def text(
    ...
    try:
        results = webs.text(
            keywords, region, safesearch, timelimit, backend, max_results
        )
        _print_data(results)
    except Exception as e:
...
@option(
    "--longitude",
    "-lon",
    help="Geographic coordinate (east-west position); if latitude and longitude are set, the other parameters are not used",
)
@option(
    "--radius",
    "-r",
