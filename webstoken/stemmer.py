...
class Stemmer:
    ...
    def is_vowel(self, char: str, prev_char: str = None) -> bool:
        ...
        return char in self.vowels or (
            char == "y" and prev_char and prev_char not in self.vowels
        )
    ...
    def count_syllables(self, word: str) -> int:
        ...
        for i, char in enumerate(word.lower()):
            if self.is_vowel(char, prev_char) and (
                i == 0 or not self.is_vowel(prev_char, word[i - 2] if i > 1 else None)
            ):
                count += 1
            prev_char = char
        return count or 1
